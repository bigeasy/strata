...
// Minimal flow control function. Even the delightful little Step is too much
// for our purposes.
function flow (callbacks) {
  var callback = callbacks.pop(), result;
  function next (result) {
    if (callbacks.length) {
      callbacks.shift()(declare, result);
    } else {
      callback(null, result);
    }
  }
  function declare (allowed) {
    allowed = allowed || function (e) { return false }
    return function (error, result) {
      if (error && !allowed(error)) {
        callback(error);
      } else {
        next(result);
      }
...
//
function createIO (directory, options) {
  ...
  function replace (page, suffix, callback) {
    ...
    flow(function (callback) {
      fs.stat(replacement, callback());
    }, function (callback, stat) {
      if (! stat.isFile()) throw new Error("not a file");
      fs.unlink(permanent, callback(ENOENT)); 
    }, function (callback) {
      fs.rename(replacement, permanent, callback());
    }, callback);
  }
  ...
  create.createLeaf = function createLeaf (address, override) {
    var page =
    { address: address
    , balancers: 0
    , cache: {}
  ...
  function writeJSON (fd, page, object, callback) {
    flow(function (callback) {
      // Get the file size or create a psuedo-stat with our cached value.
      if (page.position) {
        return { size: page.position };
      } else {
        fs.stat(fd, callback());
      }
    }, function (callback, stat) {
      ...
      function output () {
        if (offset == buffer.length) {
          done(null, page.position);
        } else {
          count = buffer.length - offset
          fs.write(fd, buffer, offset, count, page.position, function (error, written) {
  ...
  io.writeInsert = function writeInsert (fd, page, index, record, callback) {
  ...
  io.writeDelete = function writeDelete (fd, page, index, callback) {
  ...
  io.writePositions = function writePositions (fd, page, callback) {
  ...
  //
  io.readLeaf = function readLeaf (page, callback) {
    var fd, page;
    ...
    flow(function (callback) {
      fs.open(filename(page.address), "r+", callback());
    }, function (callback, fd_) {
      fs.fstat(fd = fd_, callback());
    }, function(callback, stat) {
      ...
      function input () {
        if (end) {
          ...
          if (start < 0) start = 0;
          fs.read(fd, buffer, 0, buffer.length, start, function (error, read) {
            end    -= read
            if (buffer[--read] != 0x0A) {
          ...
          splices.forEach(function (splice) {
            var index = splice[0], position = splice[1], entry = splice[2];
  ...
  //
  io.readRecord = function readRecord (page, position, callback) {
    var fn = filename(page.address), record, fd;
    flow(function (callback) {
      if (page.position) return { size: page.position };
      else fs.stat(fs, callback());
    }, function (callback, stat) {
      page.position = stat.size
      fs.open(fn, "r", callback());
    }, function (callback, fd_) {
      fd = fd_;
      var done = callback();
      (function input () {
        buffer = new Buffer(length)
        fs.read(fd, buffer, 0, buffer.length, position, function (error, read) {
          if (entry = readJSON(buffer, read)) {
            record = entry.pop();
            done(null);
          } else if (length > page.position - position) {
            done(new Error("cannot find end of record."));
          } else if (length > page.position - position) {
            length += length >>> 1
            input();
          } else {
  ...
  io.rewriteLeaf = function rewriteLeaf (page, suffix, callback) {
    var fn = filename(page.address, suffix), fd, positions = [] , cache = {}, done, index = 0;

    flow(function (callback) {
      // Open the new leaf page file and reset our file position.
      fs.open(filename, "a", 0644, callback());
    }, function (callback, fd_) {
      fd = fd_;
  ...
  //
  create.createBranch = function createBranch (address, override) {
    page =
    { address: address
    , addresses: []
    , balancers: 0
  ...
  //
  io.splice = function splice (page, offset, length, insert) {
  ...
  //
  io.create = function create (callback) {
    flow(function (callback) {
      // Create the directory if it doesn't exist.
      fs.stat(directory, callback())
    }, function (callback, stat) {
      if (! stat.isDirectory()) {
      ...
      var root, leaf;
      if (dir.filter(function (f) { return ! /^\./.test(f) }).length) {
      ...
      flow(function (callback) {
        writeBranch(root, ".replace", callback());
      }, function (callback) {
        rewriteLeaf(leaf, ".replace", callback());
      }, function (callback) {
        replace(leaf, ".replace", callback());
      }, function (callback) {
        replace(root, ".replace", callback());
      }, callback);
    }, callback);
  }
  ...
  // &mdash;
  io.open = function open (callback) {
    flow(function (callback) {
      fs.stat(directory, callback(function (e) {
        if (e.code == "ENOENT") {
      ...
      dir.forEach(function (file) {
        ...
        if (match = /^segment(\d+)$/.exec(file)) {
          nextAddress = Math.max(parseInt(match[1], 10), nextAddress);
        }
      });
    }, callback);
  }
  ...
  // &mdash;
  io.close = function close (callback) { callback(null) }
  ...
  // &mdash;
  io.lock = function lock (address, exclusive, callback) {
    ...
    //
      ...
      creator = "create" + (address < 0 ? "Leaf" : "Branch");
      page = encache(create[creator](address, { load: [] }));
    }
  ...
  function key (page, index, callback) {
    var key;
    if (page.address < 0) {
  ...
  function unwind (callback, error, value) {
    process.nextTick(function () { callback(error, value) });
  }
  ...
  function find (page, key, low, callback) {
    ...
    (function test () {
      if (low <= high) {
        ...
        key(page, mid, check(callback, function (other) {
...
// we first begin with a descent of the b&#x2011;tree, from the root, to find
// the page we want to act upon.
//
...
// parallel, but their progress can be interleaved. When we descend the tree, we
...
// structure of the b&#x2011;tree by splitting or merging pages, we *balance*
// the b&#x2011;tree.
//
...
// determine the correct child page to visit to continue our search. We lock the
// child page. We then release the lock on the parent page.
//
...
function createDescent (io, override) {
  ...
  // level will have nodes inserted or deleted. Pages at the child level will be
  // split or merged. When we reach a parent, we use `fork` to create a new
  ...
  // may be at any level of the tree. We need to hold our lock on the branch
  // page that contains the key, while still descending the tree to find the two
  // pages that need to be merged. The two pages to merge may not be immediate
  // children of the same penultimate branch page.
  //
  ...
  // the addresses current descent to the dummy page, and copy the current
  // index.
  descent.fork = function fork () {
    return createDescent(io, { page: page, exclusive: exclusive, index: index });
  }

  descent.key = function key (key) {
    return function (callback) {
      io.find(page, key, page.address < 0 ? page.ghosts : 1, callback); 
    }
  };

  descent.found = function found (key) {
    return function () {
      return page.addresses[0] != 0 && io.comparator(page.cache[page.addresses[index]],  key) == 0;
    }
  };

  descent.leftMost = function leftMost (callback) { callback(null, page.ghosts || 0) };

  // Stop when we reach a penultimate branch page.
  descent.penultimate = function () { return page.addresses[0] < 0 };

  // Stop when we reach a leaf page.
  descent.leaf = function () { return page.address < 0 }
  ...
  descent.right = function(callback) { callback(null, (page.addresses || page.positions).length - 1) };

  // All subsequent locks acquired by the descent are exclusive.
  descent.exclude = function () { exclusive = true };
  ...
  descent.upgrade = function (callback) {
    io.unlock(page);
    io.lock(page.address, exclusive = true, check(callback, function (locked) {
      page = locked;
      callback(null);
    }));
  }

  descent.descend = function (next, stop, callback) {
    (function downward() {
      var above;
      if (stop()) {
        unwind(callback);
        ...
        depth++;
        above = page;
        io.lock(above.addresses[index], exclusive, check(callback, function (locked) {
          if (unlock) io.unlock(above);
          next(check(callback, function (index_) {
            index = index_;
            unlock = true;
...
// To read records, as well as insert and delete records, they use
// ***mutator***.
//
...
// record may not actually exist in the b&#x2011;tree, in which case the
// iterator begins with the leaf page where record *would* be.
//
// #### Page by Page
//
...
// the page, but they cannot insert or delete records. By locking the pages left
// to right hand over hand, then there is no way for the tree to mutate such
// that would defeat our iteration. Leaf pages that we've visited may by edited
// by another descent after we've visited them, however.
//
...
// exactly at the millisecond that defines noon. Our iterator begins at the
// point that is either an event that occurred millisecond that defines noon, or
// else the first event that occurred after the noon millisecond.
//
...
//
function createIterator (key, descent) {
  // Iterators are initialized with the results of a descent.
  var io = descent.io
    , page = descent.page
    , sibling = null
    , iterator =
      { exclusive: descent.exclusive
      , length: page.positions.length
      , index: descent.index
      , offset: descent.index < 0 ? ~ descent.index : descent.index
      }
    ;
  
  // Get a the record at a given index from the current leaf page.
  iterator.get = function get (index, callback) {
    io.stash(page, page.positions[descent.index], pluck(callback, function (entry) {
      return entry.record;
    }));
  };

  // Go to the next leaf page, the right sibling leaf page. Returns true if
  // there is a right sibling leaf page of the current page, false if there the
  // current leaf page is the last leaf page in the b&#x2011;tree.
  iterator.next = function next (callback) {
    var next;
    ...
    if (page.right) {
      ...
      flow(function (callback) {
        ...
        // Otherwise fetch the next page.
          ...
          io.lock(page.right, exclusive, callback());
        }
  ...
  descent.indexOf = function indexOf (key, callback) {
    io.find(page, key, page.ghosts, callback);
  }

  // Unlock all leaf pages held by the iterator.
  descent.unlock = function unlock () {
    io.unlock(page);
    if (sibling) io.unlock(sibling);
  }
...
// **TODO**: Zero is a valid index for the left most leaf page.
  ...
  iterator.insert = function insert (record, k, index, callback) {
    ...
    unambiguous = unambiguous || key && io.comparator(key, k) == 0
    ...
    flow(function (callback) {
      if (! unambiguous) {
        ...
        }, function (callback, sibling_) {
          io.key(sibling = sibling_, 0, callback());
        }, function (callback, siblingKey) {
          return io.comparator(key, siblingKey) < 0;
        }, callback());
      } else {
        return unambiguous;
      }
    }, function (callback, unambiguous) {
      ...
      if (unambiguous) {
        ...
        var filename = io.filename(page.address), fd;
        flow(function (callback) {
          fs.open(filename, "a", 0644, callback());
        }, function (callback, fd_) {
          io.writeInsert(fd = fd_, page, index, record, callback());
        }, function (callback, position_) {
          // Insert the position into the page a cache the record.
          io.splice(page, index, 0, position);
          io.cacheRecord(page, position, record, key);
        ...
        // Return false since we did not insert the record.
        return unambiguous;
      }
    }, callback);
  }
  ...
  // the `page`.
  iterator.delete = function (index, callback) {
    ...
    flow(function () {
      ...
      io.writeInsert(fd = fd_, page, index, callback());
    }, function (callback, position) {
      ...
      if (ghost) {
        ...
        io.uncacheRecord(page, page.positions[index]);
        io.splice(page, index, 1);
        length = page.length
      }
      fs.close(fd, callback());
    }, callback);
...
// candidates for balance. Balancing will take place periodically, where a
...
// descend the tree once to insert or delete records form the leaf pages. We
// then descend the tree once for each split or merge of a page.
//
...
// sub&#x2011;tree. It can not be reached by the path that was used to find the
// leaf page where the delete occurred.
//
...
// no longer necessary. There may be, in fact, a descent on the way to the left
...
// the changes into account. This can avoid degenerate cases of sort where a
// leaf page at the split threshold and the application in turn inserts and
...
// fast as the operating system can append a string to a file. We'll balance
// this mess for her later.
//
...
// one one series of descents. A balance descent can assume that the tree retain
// its structure while the balance descent waits on I/O.
//
// ##### Balancer
//
...
// to collect a new round of offset counts, while we are off balancing the pages
...
// concurrently. Those changes will be reflected when the next balancer balances
// the tree.
//
...
// size, we split the page. That much is obvious. Otherwise, if the offset count
// is negative, we've deleted records. There may be an opportunity to merge, so
// we check the left and right siblings of the page to determine if a merge is
// possible.
//
// Determining a plan for a merge requires inspecting three pages, the page that
// decreased in size, plus its left and right sibling pages. We merge a page
// with the sibling that will create the smallest page that is less than or
// equal to the maximum page size.
//
// ##### Purging the Cache
//
// When inspecting page for merge, we are only interested in the count of
// records in the page. It may have been a long time since the last merge, so we
// might have accumulated a lot of pages that need to be consulted. To create a
...
// was loaded by the balancer. We also set the balancer flag, indicating that we
...
// the parent of the branch page.  We acquire an exclusive lock the branch page.
// We can release our shared lock and acquire an exclusive lock. We do not have
...
// both of our two halves may be a candidate to merge with what were the left
// and right siblings of the page before it split. There may have been less than
// half a page of records one or both both of the sides of the tree. After we
// split, we check for a chance to merge. More live lock fears, but one heck of
// a balanced tree.
//
// ### Need to Add
//
...
// pages. As we delete records, we still keep a ghost of a key. As we delete
// leaf pages, we delete the ghost keys. Branch pages become paths to nowhere.
// They don't hold their own keys, so they can't find them.  We'd have to have
// null keys in our tree. Even if we kept keys around, we're sending searches
// down a path to nowhere. There is no leaf page to visit. We get rid of these
// paths. We always balance the upper levels immediately, we perform the
// cascade. Our tree descent logic would have to account for these empty
// sub&#x2011;trees. Much better to balance and keep things orderly.
//
...
// well. What's the big deal then? How do we fix that? If it is a server
// install, we demand that you maintain your clock. If it is a desktop install,
// we can comb the entire database, because how big is it going to be?
//
...
// cannot change, because it is the balancer that alters keys. The first record
// may be deleted by editing, but a ghost of the first record is preserved for
// the key value, which is the key value of the page.
//
// When we encounter the key value in a branch page, we acquire an exclusive
// lock the branch page. We can release our shared lock and acquire an exclusive
// lock. We do not have retest conditions after the upgrade, because only the
// balancer would change the keys in a branch page, and we're the balancer.
//
...
// locks in the hand-over-hand fashion, until we reach a leaf page. We are now
// at the left sibling of the page we want to merge.
//
...
// left sibling. We lock exclusively hand-over-hand thereafter to squeeze out
// any shared locks. Our exclusive lock on the parent containing the key
// prevents another descent from entering the sub&#x2011;tree where we are
...
// merging branch pages other than the root branch page is the same regardless
// of the depth of the branch page.
//
...
// the merge page. We have to make an exception when the empty page is the left
// most page in the entire tree, which is not uncommon for time series data
// where the oldest records are regularly purged.
//
...
function createBalancer (leafSize) {
  var lengths = {}
    , operations = []
    , referneced = {}
    , balancer = {}
  ...
  // record count of the page is less than the order of the leaf page, it will
  // be test for merge. If it is greater than the order of the leaf page, it
  // will be split. Of course, if it the order of the page, it can not be
  // merged, nor should it be split.
  balancer.unbalanced = function unbalanced (page, force) {
    if (force) {
  ...
  // concurrent, though. Makes no sense to try to run more than one at a time,
  // or it doesn't make sense to run a balance when a balance is running, or
  // rather, it doesn't make sense to make it a matter of running one after each
  // insert. Horrible writing here. Do not use.
  ...
  // check every page that has been added to the balancer, regardless of whether
  // it has grown, shrunk or returned to its original reported size. If the page
  // is to be deleted, because the leaf page is empty, that negates any fussing
  // with the key. Same goes for the case where the page is to be merged.
  ...
  // where the b&#x2011;tree cannot reach a balanced state. Inserts and deletes
  // may be taking a set of pages from needing split to needing merge faster
  // than the balance plan can figure it, and the balance operations can make it
  // so.
  ...
  // are not capricious with it. We are okay with having to load the sibling
  // page lengths to check for merge. A split will lead to a subsequent balance
  // plan that will load four pages. In that regard, splits are not cheaper than
  // merges.
  ...
  // it is now too small. We could create an outer loop that keeps on
  ...
  function balance (io) {
    ...
    var ordered = {}, i, I, address, page, node;
    flow(function () {
      (function next() {
        // Convert the address back to an integer.
        var address = +(addreses.shift()), length = lengths[address], right;
        ...
        // to the node containing the previous sibling, and a reference to the
        // node containing the next sibling. If a sibling is not participating in
        // our balance plan, then its link is null. This gives us one or more
        // linked lists that reference a series of leaf pages ordered according to
        // their order in the b&#x2011;tree.
        //
        ...
        flow(function (callback) {
          ...
          if (! node = ordered[address]) {
            flow(function (callback) {
              io.lock(address, false, callback());
            }, function (callback, page_) {
              io.key(page = page_, 0, callback());
          ...
          if (! node.right && (right = ordered[page.right])) {
            node.right = right
            right.left = node
          }
          ...
          if (node.length - length < 0 && node.page.address != -1 && ! node.left) {
            var descent;
            flow(function (callback) {
              descent = createDescent(io);
              descent.descend(Descent.key(node.key), Descent.found(node.key), callback());
            }, function (callback) {
              // **TODO**: You know that this would drive you mad and cost you 3
              // days if you were a Java programmer. Encapsulation! Encapsulation!
              descent.index--
              descent.descend(Descent.right, Descent.leaf, callback());
            }, function (callback) {
              io.key(descent.page, 0, check(done, callback));
            }, function (callback, key) {
              var left =
              { page: descent.page
              , length: descent.page.length - descent.page.ghosts
              , key: key
              }
              reference(left.page);
              io.unlock(left.page);
          ...
          if (!node.right && node.page.right) {
            flow(function (callback) {
              if (right = ordered[page.right]) {
                return right;
              } else {
                var page;
                flow(function (callback) {
                  io.lock(page.right, false, callback());
                }, function (callback, page_) {
                  io.key(page = page_, 0, callback());
                }, function (callback, key) {
                  ordered[node.page.right] = right = { page: page, length: page.length - page.ghosts, key: key };
                  reference(page);
                  io.unlock(page);
                  return right;
                }, callback()); 
      ...
      // shrinking while we were waiting evented I/O. Note how we drop the page if
      // a split is not necessary.
      ...
      for (address in lengths) {
        ...
        if (difference > 0 && node.length > max) {
          // Schedule the split.
      ...
      (function next() {
        if (operations.length) {
          methods[operation.method].call(balancer, operation, check(done, next));
        } else {
          done(null);
        }
      })();
  ...
  // when it was last known to be balanced in relation to its siblings. Until we
  // can either delete it or run it through a plan where it is known to be
  // balanced, it is in an unbalanced state.
  //
  // **TK**: Docco.

  //
  function splitLeaf (context, callback) {
    // Keep track of our descents so we can unlock the pages at exit.
    var descents = [], penultimate, leaf, split;
    ...
    flow(function (callback) {
      ...
      // will be removed.
      penultimate.descend(sought, Descent.penultimate, callback());
    }, function (callback) {
      penultimate.upgrade(callback());
    }, function (callback) {
    ...
    function copier () {
      if (index < offset + length) {
        ...
        var position = split.position[index];
        flow(function (callback) {
          io.stash(split, position, callback());
        }, function (callback, object) {
    ...
    function copied() {
      ...
      io.rewriteLeaf(split, ".replace", check(callback, function () {
    ...
    function transaction () {
      flow(function (callback) {
        // Write the branches
        io.writeBranch(penultimate.page, ".pending", callback());
      }, function (callback) {
        // Now rename the last action, committing to our balance.
        io.rename(penultimate.page, ".pending", ".commit", callback());
      }, function (callback) { 
        // Rename our files to put them in their place.
        replacements.forEach(function (page) {
          io.replace(page, ".replace", callback);
        });
      }, function (callback) {
        // Add our new pages to the cache.
        uncached.forEach(function (page) { io.encache(page) } );

        // This last replacement will complete the transaction.
        io.replace(penultimate.page, ".commit", callback());
      }, check(callback, function () {
  ...
  function mergeLeaves (callback, key, unbalanced) {
    ...
    flow(function (callback) {
      ...
      penultimate.left.descend(Descent.right, Descent.penultimate, callback());
    }, function (callback) {
      penultimate.right = pivot.fork();
      penultimate.right.descend(sought, Descent.penultimate, callback());
    }, function (callback) {
      ...
      leaves.left.descend(Descent.leftMost, Descent.leaf, callback());
    }, function (callback) {
      descents.push(leaves.right = penultimate.right.fork());
      leaves.right.descend(Descent.leftMost, Descent.leaf, callback());
    }, check(callback, applicable));
    ...
    function merge () {
      ...
      var index = leaves.right.page.ghosts;
    ...
    function append () {
      ...
      io.stash(leaves.right.page, position, check(callback, function (object) {
    ...
    function appended () {
      flow(function (callback) {
        ...
        // pending unlink.
        io.rewriteLeaf(leaves.left.page, ".replace", callback());
      }, function (callback) {
        io.rename(leaves.right.page, "", ".unlink", callback());
      }, function (callback) {
        io.uncacheKey(pivot.page, pivot.page.addresses[pivot.index]);
        io.splice(penultimate.right.page, penultimate.right.index, 1);

        io.writeBranch(penultimate.right.page, ".pending", callback());
      }, function (callback) {
        ...
        io.rename(penultimate.right.page, ".pending", ".commit", callback());
      }, function (callback) {
        io.replace(leaves.left.page, ".replace", callback());
      }, function (callback) {
        io.unlink(leaves.right.page, ".unlink", callback());
      }, function (callback) {
        io.replace(penultimate.right.page, ".commit", callback());
      }, check(callback, release));
    }
...
function createStrata(options) {
  ...
  var defaults = {
    ...
    comparator: compare,
    extractor: extract };
  var io = createIO(options.directory, extend(defaults, options));

  function create (callback) { io.create(callback) }

  function open (callback) { io.open(callback) }

  function close (callback) { io.close(callback) }
  ...
  // &mdash;
  function cursor (key, exclusive, constructor, callback) {
    // In theory, we can support null keys, since we can test to see if we've
    // been provided a key value by the arity of invocation.
    sought = key.length ? Descent.key(key[0]) : Descent.leftMost;

    flow(function (callback) {
      // Descend to the penultimate branch page.
      descent = createDescent(io);
      descent.descend(sought, Descent.penultimate, callback());
    }, function (callback) {
      if (exclusive) descent.exclude()
      descent.descend(sought, Descent.leaf, callback());
    }, function (callback) {
      return new constructor(key, descent);
    }, callback);
  }

  function iterator () {
    var splat = __slice.apply(arguments, 0);
    _cursor(splat, false, Iterator, splat.pop());
  }

  function mutator () {
    var splat = __slice.apply(arguments, 0);
    _cursor(splat, true, Mutator, splat.pop());
  }

  // Create a cassette to insert into b&#x2011;tree.
  function cassette (object) {
    return { record: object, key: io.extractor(object) };
  }
  ...
  function balance (callback) { io.balancer.balance(io, callback) }
}
