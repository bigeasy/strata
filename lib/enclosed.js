...
function Descent (override) {
  ...
  return objectify.call(this, descend, exclude, key, leftMost, penultimate, leaf);
}
...
// **TODO**: Zero is a valid index for the left most leaf page.
  ...
  function remove (index, callback) {
    ...
    function written (position) {
      ...
      // the cursor is `0`, we move that forward to `1`. Otherwise, we uncache
      // and splice the record.
      if (ghost) {
        page.ghosts++;
        offset || offset++;
      } else {
...
function Balancer () {
  ...
  function balance () { balancer.balance() }
