...
function Strata (directory, options) {
  ...
  function stats () { return { size: size } }
  ...
  function writeJSON (fd, page, object, callback) {
    ...
    function sent(written) {
      ...
      offset += written
      if (offset == buffer.length) callback(null, page.position);
      else send();
    }
  }
  ...
  function rewriteLeaf (page, suffix, callback) {
    ...
    function rewrite () {
      var position = position.shift(), object;
    ...
    function list (files) {
      files.forEach(function (file) {
        ...
        if ($ = /^segment(\d+)$/.exec(file)) {
          nextAddress = Math.max(+($[1]), nextAddress);
        }
      });
      callback(null);
    }
  ...
  function find (page, key, low, callback) {
    var high = page.length - 1, check = validator(callback);
...
function Descent (override) {
  ...
  function leftMost (callback) { callback(null, page.ghosts || 0) }
  ...
  return objectify.call(this, descend, key, leftMost, penultimate, leaf);
}
...
//
function Cursor (exclusive, key, page, index) {
  ...
  objectify.call(this, unlock, indexOf, get, next, _offset, _length);
...
// **TODO**: Zero is a valid index for the left most leaf page.
  ...
  //
  function insert (record, k, index, callback) {
...
function createBalancer (leafSize) {
  ...
  function splitLeaf (context, callback) {
    ...
    function copied() {
      ...
      rewriteLeaf(split, ".replace", check(callback, function () {
  ...
  function mergeLeaves (callback, key, unbalanced) {
    ...
    function appended () {
      flow(function (callback) {
        ...
        // pending unlink.
        rewriteLeaf(leaves.left.page, ".replace", callback());
      }, function (callback) {
  ...
  function cursor (key, exclusive, callback) {
    ...
    sought = key.length ? descent.key(key[0]) : descend.leftMost;
  ...
  return objectify.call(this, create, open, iterator, stats, close);
}
